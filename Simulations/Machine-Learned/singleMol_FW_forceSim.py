...
src = root_dir / "saved_simulations" / "ForceSim.json"
...
def ml_co2_framework_forces(co2_pos_np) -> np.ndarray:
    """
...
logger = ForceLogger(fw_tree, fw_Z_np=fw_Z_np, co2_Z_all=co2_Z_all)

def run_step(step, record_every=10, useClassicalForces=True):
    """
    Parameters
    ----------
    step : int
    ...
    record_every : int
        Interval for logging force statistics.
    useClassicalForces : bool
        Whether to apply classical (LJ) forces or ANI-2x forces during the integration.
    """
    md.runFirstHalfStep()                            # Velocity-Verlet: drift
...
for step in range(NumberOfCycles):
    run_step(step, record_every=10, useClassicalForces=True)
    if step % 100 == 0:
